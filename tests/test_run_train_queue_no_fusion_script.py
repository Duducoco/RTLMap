...
def _install_queue_fixture(tmp_path: Path) -> tuple[Path, Path]:
    queue_script = tmp_path / "run_train_queue_no_fusion.sh"
    queue_script.write_text(
        (PROJECT_ROOT / "run_train_queue_no_fusion.sh").read_text(encoding="utf-8"),
        encoding="utf-8",
    )
...
def test_no_fusion_queue_runs_all_datasets_with_isolated_experiments(
    tmp_path: Path,
) -> None:
    ...
    assert [row[2] for row in rows] == [
        f"{dataset}-4coverage-no-fusion" for dataset in EXPECTED_DATASETS
    ]
    assert {row[3] for row in rows} == {"checkpoints/no_fusion"}


def test_no_fusion_queue_help_reports_architecture_and_order() -> None:
    result = subprocess.run(
        ["bash", str(PROJECT_ROOT / "run_train_queue_no_fusion.sh"), "--help"],
        cwd=PROJECT_ROOT,
        capture_output=True,
        text=True,
        check=False,
