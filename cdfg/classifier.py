...
class CellClassifier:
    ...
    # 时序类
    SEQUENTIAL_CELLS = {
        '$dff', '$dffe', '$adff', '$adffe', '$asdff', '$asdffe',
        '$sdff', '$sdffe', '$sdffce', '$dlatch', '$adlatch', '$dlatchsr',
        '$sr', '$ff', '$aldff', '$aldffe'
    }

    # 存储器类
    MEMORY_CELLS = {'$mem', '$mem_v2', '$memrd', '$memrd_v2', '$memwr', '$memwr_v2', '$meminit', '$meminit_v2'}

    # 算术类
    ARITHMETIC_CELLS = {'$add', '$sub', '$mul', '$div', '$mod', '$divfloor', '$modfloor', '$pow', '$neg', '$pos'}
    ...
    @staticmethod
    def parse_source_location(src_attr: str) -> Tuple[str, int]:
        """
        解析 src 属性，提取源文件名和行号

        src 属性格式示例：
        - "..\\designs\\cv32e40p\\rtl\\cv32e40p_int_controller.sv:96.29-96.51"
        - "sv:96.29-96.51|sv:96.5-147.39" (多位置用 | 分隔)

        Args:
            src_attr: Yosys JSON 中的 src 属性值

        Returns:
            (文件名, 行号) 元组，如果解析失败返回 ("", 0)
        """
        if not src_attr:
            return "", 0

        # 取第一个位置（| 分隔的第一部分）
        first_loc = src_attr.split('|')[0].strip()
