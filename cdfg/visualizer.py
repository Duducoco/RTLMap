...
class CDFGVisualizer:
    ...
    # 节点样式配置
    NODE_STYLES = {
        NodeType.INPUT: {'shape': 'ellipse', 'fillcolor': 'lightgreen', 'style': 'filled'},
        NodeType.OUTPUT: {'shape': 'ellipse', 'fillcolor': 'lightcoral', 'style': 'filled'},
        NodeType.CONSTANT: {'shape': 'diamond', 'fillcolor': 'lightyellow', 'style': 'filled'},
        NodeType.SEQUENTIAL: {'shape': 'box', 'fillcolor': 'lightblue', 'style': 'filled,bold'},
        ...
        NodeType.MEMORY: {'shape': 'box3d', 'fillcolor': 'lightgray', 'style': 'filled'},
        NodeType.SHIFT: {'shape': 'box', 'fillcolor': 'honeydew', 'style': 'filled'},
        NodeType.COMBINATIONAL: {'shape': 'box', 'fillcolor': 'white', 'style': 'filled'},
    ...
    def render(self, output_file: str = 'cdfg', format: str = 'svg',
               show_ports: bool = True, show_widths: bool = True,
               rankdir: str = 'TB', separate_control: bool = False,
               engine: str = 'auto', show_coverage: bool = False) -> str:
        """
        渲染 CDFG
        ...
        if engine == 'auto':
            ...
            if node_count > 100 or edge_count > 300:
                engine = 'sfdp'
                print(f"[INFO] 图较大 ({node_count} 节点, {edge_count} 边)，使用 sfdp 引擎加速渲染")
            else:
                engine = 'dot'
    ...
    def _add_coverage_legend(self, dot: graphviz.Digraph):
        ...
        with dot.subgraph(name='cluster_legend') as legend:
            ...
            # 创建示例边
            legend.edge('legend_start', 'legend_covered',
                       **self.COVERAGE_EDGE_STYLES['covered'])
            legend.edge('legend_start', 'legend_uncovered',
                       **self.COVERAGE_EDGE_STYLES['uncovered'])
            legend.edge('legend_start', 'legend_unknown',
                       **self.COVERAGE_EDGE_STYLES['unknown'])
    ...
    def _render_unified(self, dot: graphviz.Digraph, show_ports: bool, show_widths: bool,
        ...
        for node_id, node in self.cdfg.nodes.items():
            ...
            style = self.NODE_STYLES.get(node.node_type, self.NODE_STYLES[NodeType.UNKNOWN])
        ...
        for edge in self.cdfg.edges:
            ...
            label = self._create_edge_label(edge, show_ports, show_widths, show_coverage)
            ...
            if show_coverage:
                ...
                # 使用默认边类型样式
                style = self.EDGE_STYLES.get(edge.edge_type, self.EDGE_STYLES[EdgeType.DATA]).copy()

            dot.edge(safe_source, safe_target, label=label, **style)

    def _render_separated(self, dot: graphviz.Digraph, show_ports: bool, show_widths: bool,
                          show_coverage: bool = False):
        """分离控制路径和数据路径渲染"""
        # 数据路径子图
    ...
    def _create_edge_label(self, edge: Edge, show_ports: bool, show_widths: bool,
